...
class HoudiniSceneCollector(HookBaseClass):
    """
    Collector that operates on the houdini scene
    """
    ...
    def process_file(self, parent_item, path):
        ...
        item = super(HoudiniSceneCollector, self).process_file(
            parent_item, path)

        if path.endswith(".hip") or path.endswith(".hipnc"):

            item.type = "file.houdini"
            item.display_type = "Houdini File"
